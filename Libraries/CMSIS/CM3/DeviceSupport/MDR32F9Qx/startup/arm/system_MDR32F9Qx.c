...
/**
  * @brief  SetSysClock setting system clock
  * @note   None
  * @param  None
  * @retval None
  */
...
{
  __IO uint32_t startCounter = 0, HSEstatus=0, PLLstatus;
	
	//Set HSEON bit
	MDR_RST_CLK->HS_CONTROL |= (uint32_t)0x00000001;	

  /* Wait until HSE is ready or time out is occure */
  do
  {
    HSEstatus = MDR_RST_CLK->CLOCK_STATUS & ((uint32_t)1<<0x02);
//    startCounter++;
  } while ((startCounter < HSEonTimeOut) && (HSEstatus == (uint32_t)0x00));
	
	startCounter=0;
  /* Get the flag status on proper position */
  if ((MDR_RST_CLK->CLOCK_STATUS & (uint32_t)0x04) != (uint32_t)0x00)
  {
  ...
  else
  {
	...
	if (HSEstatus == (uint32_t)0x01)
	{
		...
		MDR_RST_CLK->CPU_CLOCK |= ((uint32_t)0x00000002);							//Set the CPU_C1_SEL bits
		MDR_RST_CLK->PLL_CONTROL |= (CPU_PLLmul10 << 8);								//PLLCPUMUL=10
		...
		/* Enables the CPU_PLL */
		MDR_RST_CLK->PLL_CONTROL |= (uint32_t)0x04;
  /* Wait until CPUPLL is ready or time out is occure */
		do
		{
			PLLstatus = MDR_RST_CLK->CLOCK_STATUS & ((uint32_t)1<<0x01);
			startCounter++;
		} while ((startCounter < PLLCPUonTimeOut) && (PLLstatus == (uint32_t)0x00));
	
			/* Enables the RST_CLK_PCLK_EEPROM */
		MDR_RST_CLK->PER_CLOCK |= (uint32_t)0x08;
		MDR_EEPROM->CMD |= ((uint32_t)0x00000018);
		
		/* Select the CPU_PLL output as input for CPU_C3_SEL */
		MDR_RST_CLK->CPU_CLOCK |= ((uint32_t)0x00000004);
		
		/* Set CPUClk Prescaler */
		MDR_RST_CLK->CPU_CLOCK |= ((uint32_t)0x00000000);
		/* Clear CPU_C3_SEL bits */
		MDR_RST_CLK->CPU_CLOCK &= (~(uint32_t)(0x03 << 8));
		/* Set the CPU_C3_SEL bits */
		MDR_RST_CLK->CPU_CLOCK |= ((uint32_t)0x00000100);
